...
14. [Data Observations](#14-data-observations)
...
1. **Drop the 48 duplicate `users.email` unique constraints/indexes.** Keep exactly one. Prevents further write amplification and stops the growth (it went to 49 — it will keep climbing on each ORM sync).
   ```sql
   -- Inspect, then drop the duplicates (keep users_email_key):
   ALTER TABLE users
     DROP CONSTRAINT users_email_key1,
     DROP CONSTRAINT users_email_key2;  -- ... through users_email_key49
   ```
2. **Stop the root cause.** Disable `sequelize.sync({ alter: true })` in any long-running/production path; switch to explicit migrations (e.g. `sequelize-cli`/Umzug). The duplicate constraints are re-created every time `alter: true` runs against an existing `unique: true` column.

### 🟠 High

3. **Add an index on `options.questionId`** (7,239 rows, the hottest FK):
   ```sql
   CREATE INDEX idx_options_question_id ON options ("questionId");
   ```
4. **Drop the 4 redundant junction unique indexes** that duplicate the composite PK (`question_subjects_question_id_subject_id`, and the chapters/subject_categories/exam_categories equivalents).
5. **Resolve the `subjects.questionId` anomaly.** If subjects are a shared dimension (they are — 1 row, tagged via junction), drop the column:
   ```sql
   ALTER TABLE subjects DROP COLUMN "questionId";
   ```

### 🟡 Medium

6. **Add supporting indexes** on `question_images.questionId`, `auth.userId`, and the reverse-lookup FK columns of junctions (`subjectId`, `chapterId`, `topicId`, `subjectCategoryId`, `examCategoryId`) if "find questions by topic/chapter" queries exist.
7. **Add CHECK/partial-unique constraints for answer integrity**, e.g. ensure a `Single`-type question has exactly one correct option:
   ```sql
   CREATE UNIQUE INDEX one_correct_per_single_question
     ON options ("questionId") WHERE "isCorrect";  -- combined with app enforcement of optionType
   ```
8. **Decide `exams`' fate** — either wire it into the model or drop the orphan table + sequence.
9. **Unify PK width** — either make taxonomy `id`s `bigint` or `questions`/`options` `int`; document the choice.

### 🟢 Low

10. **Add DB defaults for timestamps** (`DEFAULT now()`), or an `updatedAt` trigger, so out-of-ORM inserts don't violate `NOT NULL`.
11. **Case-insensitive email** — store email as `citext` (enable the extension) or add a `lower(email)` unique index to stop `A@x.com`/`a@x.com` duplicates.
12. **Add table/column comments** (`COMMENT ON ...`) so the schema is self-documenting at the source.
13. **Consider `text` over `varchar(255)`** for `imageLink` and free-text names to avoid silent truncation.
